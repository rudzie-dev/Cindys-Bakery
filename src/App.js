...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf7] font-sans text-slate-800 antialiased selection:bg-blue-100">
                ...
                <div className="space-y-4 pb-4">
                  {cart.map((item) => (
                    <div key={item.id} className="flex gap-4 p-4 rounded-2xl border border-slate-100 hover:bg-blue-50/30 transition-colors">
                      <div className="w-16 h-16 bg-blue-50 rounded-xl flex items-center justify-center text-blue-300 shrink-0">
                        <Cake size={24} />
                      </div>
              ...
              <div className="p-6 border-t border-slate-100 bg-white">
                <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
                  <button 
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${deliveryMethod === 'pickup' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-500'}`}
                  >
                    <Store size={14} /> Pickup
                  </button>
                  <button 
                    onClick={() => setDeliveryMethod('delivery')}
                    className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${deliveryMethod === 'delivery' ? 'bg-white text-blue-900 shadow-sm' : 'text-slate-500'}`}
                  >
                    <Truck size={14} /> Delivery (+R{DELIVERY_FEE})
                  </button>
                </div>
                ...
                <div className="space-y-2 mb-6 text-sm">
                  ...
                  <div className="flex justify-between items-end pt-2 border-t border-slate-50">
                    <span className="text-slate-950 font-bold">Total</span>
                ...
                <button 
                  ...
                  className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100"
                >
      ...
      {/* Hero */}
      <section id="about" className="relative min-h-[95vh] flex items-center justify-center overflow-hidden">
      ...
      <section id="menu" className="py-32 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-20 max-w-2xl mx-auto">
          ...
          <div className="flex justify-center gap-2 mt-8">
            ...
            <div className="h-1 w-4 bg-blue-200 rounded-full" />
          </div>
        </div>
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  {inCart ? (
                    ...
                    <button 
                      ...
                      className="w-full bg-blue-50 text-blue-700 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-blue-600 hover:text-white transition-all active:scale-95 group/btn"
                    >
      ...
      <section id="contact" className="py-32 px-6 bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              <div className="inline-flex items-center gap-2 text-blue-600 font-black tracking-widest uppercase text-xs mb-8">
                <Heart size={14} fill="currentColor" /> Hand-made Tradition
              </div>
              <h2 className="text-5xl md:text-7xl font-serif text-blue-950 mb-8 leading-[1.1]">
                Real Baking <br/> For <span className="text-blue-700 italic underline decoration-blue-100 underline-offset-8">Real People</span>
              </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-blue-50 rounded-2xl text-blue-600 shadow-sm"><MapPin size={20}/></div>
                  <div>
                    <h4 className="font-bold text-blue-950 text-sm mb-1">Pick-up Location</h4>
                    <p className="text-slate-500 text-sm">5084 Malandela Road, Section B, Ezakheni</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="p-3 bg-blue-50 rounded-2xl text-blue-600 shadow-sm"><Clock3 size={20}/></div>
                  <div>
                    <h4 className="font-bold text-blue-950 text-sm mb-1">Baking Windows</h4>
                    <p className="text-slate-500 text-sm">Mon - Sat: Orders ready by 9am</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="bg-[#fcfaf7] p-8 md:p-14 rounded-[3.5rem] border border-blue-100 shadow-2xl relative">
              <div className="absolute -top-6 -right-6 bg-yellow-400 text-blue-950 font-black px-6 py-4 rounded-3xl shadow-xl rotate-12 hidden md:block">
                Bulk Orders <br/> Welcome!
              </div>
              <h3 className="text-3xl font-serif text-blue-950 mb-8">Ready to order?</h3>
              <div className="space-y-4">
                <a href={`tel:${phoneNumber}`} className="flex items-center justify-between p-6 bg-white rounded-3xl hover:border-blue-400 border border-transparent transition-all hover:shadow-xl group">
                  <div className="flex items-center gap-5">
                    <div className="p-3 bg-blue-100 text-blue-600 rounded-2xl group-hover:scale-110 transition-transform"><Phone /></div>
                    <div className="text-left">
                      <p className="font-bold text-blue-950">Call Sindy</p>
                      <p className="text-xs text-slate-400">Speak directly to the baker</p>
                    </div>
                  </div>
                ...
                <div className="p-8 bg-blue-900 rounded-[2.5rem] text-white text-center mt-10 shadow-2xl shadow-blue-900/20">
                  <p className="font-serif italic text-xl mb-3 text-blue-100">Planning a Wedding or Funeral?</p>
                  <p className="text-blue-300/80 text-sm mb-6 leading-relaxed">I specialize in providing large quantities for local ceremonies. Contact me for a custom quote on bulk scone buckets.</p>
                  <a href={`https://wa.me/27${phoneNumber.substring(1)}?text=I'd like to discuss a custom order`} className="inline-block bg-white text-blue-900 px-8 py-3 rounded-full font-bold text-sm hover:bg-blue-50 transition-colors">Special Quote</a>
                </div>
              </div>
            </div>
          </div>
